...
gboolean on_close_clicked(GtkWidget *widget, GdkEvent *event, gpointer user_data) {
    ...
    // Only allow main window (id 0) to terminate the application
    // For secondary windows, just hide them instead of destroying
    if (self && self->GetId() != 0) {
        gtk_widget_hide(widget);
        return TRUE; // Prevent default destruction
    }
    // For main window, allow normal destruction which will terminate the app
...
FlutterWindow::FlutterWindow(
  ...
  g_signal_connect(G_OBJECT(window_), "delete-event", G_CALLBACK(on_close_clicked), this);
